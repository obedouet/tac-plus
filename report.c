...
#endif
{
    ...
    /* Olivier BEDOUET: If debug or logfile */
    if (debug || logfile) {
	int logfd;
	if (logfile == NULL)	/* Olivier BEDOUET: We are in debug mode */
	...
	if (logfd >= 0) {
	    ...
	    /* 2010/02/22: Moved to here */
	    return; /* Olivier BEDOUET: If we have a logfile, don't use syslog */
	}
	else /* Olivier BEDOUET: if can't open logfile, do a report do syslog */
	{
